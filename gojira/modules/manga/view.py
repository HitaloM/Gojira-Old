...
@Gojira.on_callback_query(filters.regex(r"^manga characters (\d+) (\d+) (\d+)"))
@use_chat_language()
async def manga_view_characters(bot: Gojira, callback: CallbackQuery):
    ...
    async with anilist.AsyncClient() as client:
        ...
        characters_text = ""
        characters = sorted(manga.characters, key=lambda character: character.id)
        ...
        if page > 1:
            page_buttons.append(
                ("⬅️", f"manga characters {manga_id} {user_id} {page - 1}")
            )
        if not page + 1 == pages:
            page_buttons.append(
                ("➡️", f"manga characters {manga_id} {user_id} {page + 1}")
