...
@Gojira.on_callback_query(filters.regex(r"^popular character (?P<page>\d+)"))
@use_chat_language()
async def character_popular(bot: Gojira, callback: CallbackQuery):
    ...
    async with httpx.AsyncClient(http2=True) as client:
        ...
        if data["data"]:
            ...
            layout = Pagination(
                suggestions,
                item_data=lambda i, pg: f"character {i.id}",
                item_title=lambda i, pg: i.name.full,
                page_data=lambda pg: f"popular character {pg}",
            )
